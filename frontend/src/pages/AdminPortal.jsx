...
const AVAILABLE_MARKETPLACES = [
  "AMAZON_SC", "FLIPKART", "MYNTRAV4", "TATACLIQ", "NYKAA",
  "MEESHO", "AJIO", "SNAPDEAL", "EBAY", "SHOPIFY",
  "WOOCOMMERCE", "MAGENTO", "BIGCOMMERCE", "ZALANDO", "ASOS",
  "WALMART", "TARGET", "BESTBUY", "ALIEXPRESS", "SHEIN"
];
...
const AdminPortal = () => {
  ...
  const [newClient, setNewClient] = useState({ clientName: '', username: '', password: '', themeColor: randomColor(), pod: '', dbId: '', marketplaces: [] });
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [errorMsg, setErrorMsg] = useState('');
  const [clientToDelete, setClientToDelete] = useState(null);
  const [editingClient, setEditingClient] = useState(null);
  const [editForm, setEditForm] = useState({ password: '', marketplaces: [] });
  ...
  const handleAddClient = async (e) => {
    ...
    if (success) {
      ...
      setNewClient({ clientName: '', username: '', password: '', themeColor: randomColor(), pod: '', dbId: '', marketplaces: [] });
    } else {
      setErrorMsg(message);
    }
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    setIsSubmitting(true);
    setErrorMsg('');
    const { success, message } = await updateClient(editingClient._id, editForm);
    setIsSubmitting(false);
    if (success) {
  ...
  const handleMarketplaceChangeSelect = (e, isEdit = false) => {
    const selectedOptions = Array.from(e.target.selectedOptions, option => option.value);
    if (isEdit) {
      setEditForm(prev => ({ ...prev, marketplaces: selectedOptions }));
    } else {
      setNewClient(prev => ({ ...prev, marketplaces: selectedOptions }));
    }
  };

  const startEdit = (client) => {
    setEditingClient(client);
    setEditForm({ password: '', marketplaces: client.marketplaces || [] });
    setErrorMsg('');
  };
  ...
  return (
    <>
      <div className="page-wrapper">
        <div className="mb-4 pt-3 d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 stagger-1">
        <div>
      ...
      {showAddForm && (
        <div className="increff-card bg-white p-4 shadow-sm border-0 rounded-1 mb-4 stagger-2 border-top border-primary border-4" style={{ borderTopWidth: '4px !important' }}>
      ...
      {PODS.map(pod => {
        ...
        return (
          <div key={pod} className="mb-4 stagger-3">
            <div
      ...
      {/* Modals placed outside page-wrapper so they are perfectly centered on the screen viewport */}
      {clientToDelete && (
        <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center animate-fade-in" style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 9999 }}>
          <div className="bg-white p-4 rounded-2 shadow-lg" style={{ maxWidth: '400px', width: '90%' }}>
            ...
            <div className="d-flex justify-content-end gap-2">
              <button className="btn btn-light border fw-bold inc-text-xs uppercase tracking-widest px-3 py-2" onClick={() => setClientToDelete(null)}>Cancel</button>
              <button className="btn btn-danger fw-bold inc-text-xs uppercase tracking-widest px-3 py-2" onClick={executeDelete}>Delete Permanently</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingClient && (
        <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center animate-fade-in" style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 9999 }}>
          <div className="bg-white p-4 rounded-2 shadow-lg" style={{ maxWidth: '500px', width: '90%' }}>
            <h5 className="fw-bold text-gray-800 d-flex align-items-center gap-2 mb-3">
              <i className="fa-solid fa-pen-to-square text-primary"></i> Edit {editingClient.clientName}
            </h5>
            {errorMsg && <div className="alert alert-danger py-2 inc-text-sm">{errorMsg}</div>}
            <form onSubmit={handleEditSubmit} className="row g-3">
              <div className="col-12">
                <label className="form-label inc-text-xs fw-bold text-secondary uppercase">New Password (Leave blank to keep current)</label>
                <input type="text" className="form-control inc-text-sm shadow-none focus-primary"
                  value={editForm.password} onChange={e => setEditForm({ ...editForm, password: e.target.value })} placeholder="New Password" />
              </div>
              <div className="col-12 mt-4 text-end">
                <button type="button" className="btn btn-light border fw-bold inc-text-xs uppercase tracking-widest me-2 px-4 py-2" onClick={() => setEditingClient(null)} disabled={isSubmitting}>Cancel</button>
                <button type="submit" className="btn btn-primary fw-bold inc-text-xs uppercase tracking-widest px-4 py-2" disabled={isSubmitting}>
                  {isSubmitting ? <><i className="fa-solid fa-circle-notch fa-spin me-2"></i> Saving...</> : 'Save Changes'}
                </button>
              </div>
            </form>
          </div>
